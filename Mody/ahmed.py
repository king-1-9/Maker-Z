...
async def start(client, message):
   if message.from_user.username in DEVS:
     ...
     return await message.reply_text("اهلا بيك ف مصنع بوتات زد إي ميوزك", reply_markup=kep)
   kep = ReplyKeyboardMarkup([["صنع بوت", "حذف بوت"], ["السورس"]], resize_keyboard=True)
   await message.reply_text("اهلا بيك ف مصنع بوتات زد إي ميوزك", reply_markup=kep)
...
async def alivehi(client: Client, message):
    keyboard = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton("𝙶𝚁𝙾𝚄𝙿️", url=f"https://t.me/UI_XB"),
                InlineKeyboardButton("𝚂𝙾𝚄𝚁𝙲𝙴️", url=f"https://t.me/UP_UO"),
            ],
            [
                 InlineKeyboardButton(f"𐇮 𝑴𝑶𝑫𝒀 𖠮𝑺🚸𝑻𝑴𖠮 آلـۘهہؚيـٰـ‌ُـُ໋۠بـ໋ۘ۠ه 𐇮", url=f"https://t.me/UP_UO")
            ]
        ]
    )

    await message.reply_photo(
        photo="https://telegra.ph/file/f333bdf687f84fdc6cc0b.jpg",
        caption="",
...
async def makedahmed(client, message):
  if not message.from_user.username in DEVS:
    for x in Bots:
...
async def deletbotahmed(client, message):
   if not message.from_user.username in DEVS:
     for x in Bots:
         bot = x[0]
         if x[1] == message.from_user.id:       
...
async def botahmed(client, message):
  if not message.from_user.username in DEVS:
   return
  o = 0
  text = "قايمه البوتات\n"
